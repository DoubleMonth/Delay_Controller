...
uint16_t getKey(keysTypedef_t *keyS)
{
    ...
    {
        if(!GPIO_ReadInputPin((GPIO_TypeDef*)keyS->singleKey[i].keyPort,keyS->singleKey[i].keyGpio))
        {
...
*/
keyTypedef_t keyInitOne(GPIO_TypeDef * keyPort, GPIO_Pin_TypeDef keyGpio, gokitKeyFunction shortPress, gokitKeyFunction longPress)
{

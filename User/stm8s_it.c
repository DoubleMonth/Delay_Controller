...
/**
  ...
  * @retval None
  */
extern struct INTERRUPT_STATUE interrupt_statue;
extern struct FUNCTION_PARAM function_param;
extern struct BEEP_INFOR beep_infor;
extern struct DELAY_STATUE delay_statue;
...
INTERRUPT_HANDLER(EXTI_PORTB_IRQHandler, 4)
{
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */
    if(0==function_param.switch_)
    return ;
    
    if(!GPIO_ReadInputPin(INT1_GPIO,INT1_PIN))//下降沿触发
    {
        interrupt_statue.int1_full_counter=3;
    ...
    if((GPIO_ReadInputPin(INT1_GPIO,INT1_PIN)))//上升沿触发
    {
        interrupt_statue.int1_rise_counter=3;
        full_flag=0;
    //    delay_statue.int1_delay_counter=0;
    }
}
